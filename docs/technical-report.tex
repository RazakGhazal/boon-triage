...
% --- compat shim: this TeX kernel has the plain socket API (\NewSocket) but
% predates the tagging wrappers newer tcolorbox calls at load time; undefined,
% those calls typeset their arguments as literal junk on page 1. Route the
% wrappers to real sockets (so \UseTaggingSocket finds its registers) and
% no-op the tagpdf bits; \providecommand yields once the kernel catches up.
\providecommand\NewStructureName[1]{}
\providecommand\AssignStructureRole[2]{}
\providecommand\NewTaggingSocket[2]{\NewSocket{tagsupport/#1}{#2}}
\providecommand\NewTaggingSocketPlug[3]{\NewSocketPlug{tagsupport/#1}{#2}{#3}}
\providecommand\AssignTaggingSocketPlug[2]{}% leave sockets on their noop default
...
\textbf{triage, action, and never knowing whether the help worked}. This system produces a \textbf{two-tier
action queue} per facilitator --- calls capped at what a human can do, drafted WhatsApp messages
uncapped, so every quiz-failer leaves with a ready action --- by fusing a transparent rule engine
(the numbers, backtested at \textbf{AUC 0.91} against Quiz-1 outcomes) with an LLM that
\emph{reads} the Arabic facilitator notes (measured at \(\kappa=0.80\) against 75 human gold
labels). The measurement layer claims only what the data supports: only \textbf{44\%} of
quiz-failers have any logged contact since the quiz, and of the contacts that exist, just 11\%
were followed by re-engagement --- a descriptive gap (no control group) that motivates the
one-field outcome log in the roadmap. Every design choice is grounded in the early-warning and
behavioural-nudge evidence base and stated with its limitations.
\end{abstract}
...
\caption{Quiz-1 failure by learning track --- track is nearly destiny. (One further
Remedial ``0'' is an exam-day \emph{absence}, not a failure --- counted separately.)}
\begin{tabular}{lrr}
\toprule
...
This is precisely the capacity problem the system is built to relieve --- and the reason the queue caps
\textbf{only the expensive actions} per facilitator (drafted messages scale with need, facilitator
minutes do not), rather than dumping a global list.
\end{insight}
...
\node[draw=amber,dashed,rounded corners,align=left,fill=amber!5,inner sep=5pt,
  ...
  (v2){\textbf{measurement layer} (\texttt{run\_v2})\\ \texttt{backtest.py} --- Day-9 ranking vs Quiz 1\\ \texttt{eval\_extractor.py} --- vs 75 gold labels\\ \texttt{effectiveness.py} --- descriptive outcomes\\ \texttt{fairness.py} --- track + coverage equity\\ $\rightarrow$ \texttt{output\_v2.py} reports};
...
\begin{table}[h]\centering\footnotesize
\caption{Files and responsibilities (\textasciitilde2{,}300 lines total).}
\begin{tabular}{llr}
\toprule
File & Job & Lines \\
\midrule
\texttt{config.py} & tunables, \texttt{.env} loader, the case calendar & 73 \\
\texttt{ingest.py} & 3 CSVs $\rightarrow$ StudentRecord; every trap, once; dated notes & 219 \\
\texttt{risk.py} & NEED: 4 signals + cliff $\rightarrow$ tier + reasons (no AI) & 84 \\
\texttt{notes.py} & the LLM seam: Gemini reads threads $\rightarrow$ NoteState & 281 \\
\texttt{drafts.py} & template MSA drafts for the un-noted majority (no AI) & 67 \\
\texttt{decide.py} & fusion: NEED\,$\times$\,STATE, 3 rule families $\rightarrow$ ActionRow & 171 \\
\texttt{output.py} & two-tier queue, KPI counters, contact log, CSV/JSON/HTML & 368 \\
\texttt{pipeline.py} & orchestrator: run / run\_v2 / lift & 114 \\
\texttt{backtest.py} & Day-9 ranking backtest vs Quiz-1 outcomes & 88 \\
\texttt{eval\_extractor.py} & note-reader vs the human gold labels & 104 \\
\texttt{effectiveness.py} & descriptive post-contact outcomes (+caveats) & 135 \\
\texttt{fairness.py} & track audit + facilitator coverage equity & 86 \\
\texttt{output\_v2.py} & write the measurement reports & 133 \\
\texttt{tests/test\_cast.py} & 23 acceptance tests (traps, guards, KPI invariants) & 193 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{lstlisting}
You read one facilitator's note THREAD about ONE student (short, informal Saudi-dialect
Arabic, chronological, each line dated [Day N]). You never see the metrics -- a rules
engine scores those. Report what the PROSE says, weighting the most recent days. JSON:
...
ranked into \textbf{two tiers}: calls/1-on-1s capped at the top-$N$ per facilitator (facilitator time is
the scarce resource), and ready-to-send drafts \emph{uncapped} --- the largest classroom has 16
quiz-failers, so any flat top-8 list would cap its coverage at 50\% forever. Every surfaced student
...
Every facilitator note is a dated contact; the metrics are daily. Descriptively, of \textbf{75 logged
contact threads: only 11\% were followed by re-engagement, 11\% kept declining, and 13\% were logged
too late to act} (final-day notes). This is \emph{not} a causal estimate --- there is no control group,
and notes are written at dips, which revert (regression to the mean). What it does establish is worse:
\textbf{from current logs, contact effectiveness is unknowable} --- which is exactly the argument for
the one-field outcome log (``did it land?'') in the roadmap.
\end{insight}

\begin{itemize}
\item \textbf{Note-reader, measured against humans:} all 75 note threads were hand-labelled from the
notes alone, against a written codebook, \emph{before} the extractor ran (\texttt{eval/}). v3.1 agrees
with the gold labels at \(\kappa=0.80\) (87\% strict, 97\% lenient) --- and the eval earned its keep
by catching a real bug first: v3 read ``vanished + unreachable parents'' as \texttt{needs\_help}
...
(S005, S023, \dots).
\item \textbf{Measurement without a holdout:} denying triage to a live classroom is ethically wrong
and, at one cluster, statistically empty. Instead, three pre-registered designs
(\texttt{docs/EVAL\_PLAN.md}): a Day-20 primary readout (failer contact rate vs the measured 44\%
baseline), a regression discontinuity at the capacity cutoff (the top-\(k\) call list creates
near-identical students on either side of an arbitrary line), and a stepped-wedge rollout for
20\,$\to$\,100 campuses --- the deployment order \emph{is} the experiment.
\end{itemize}
...
\emph{within-track} report view keeps the most-at-risk Standard/Accelerated students visible for review
--- and Remedial membership never becomes a self-entrenching re-flag. The audit also tracks the equity
axis specific to a notes-reading system: per-facilitator note coverage (21--55\%, and the largest
classroom is the least documented), so note-informed care cannot silently concentrate where
documentation happens to be best. This directly answers the
...
exactly why the rollout is pre-registered as a stepped wedge rather than assumed to transfer. ``Two-way beats one-way'' and ``nudges survive at scale'' did not
...
\item \textbf{22/23 acceptance tests} pass --- the demo cast plus the system invariants: every
quiz-failer leaves with a ready action; the cap binds only calls/1-on-1s; the faithfulness gate blocks
unevidenced and fabricated claims; the Day-9 backtest stays predictive.
\item \textbf{Ranking backtested:} scored on days 1--9 only (pre-quiz clock), the risk score ranks who
actually failed Quiz 1 at \textbf{AUC 0.911} (precision@66 = 0.85).
\item \textbf{LLM lift:} the notes changed the outcome for \textbf{18 of 75} noted students vs.\
rules-only (9 escalated, 8 de-escalated to hand capacity back, 1 to human review).
\item \textbf{Note-reader vs.\ human gold labels:} \(\kappa=0.80\), 87\% strict / 97\% lenient
(\(n=75\), single-rater, codebook-adjudicated).
\item \textbf{Contact outcomes (descriptive):} 11\% re-engaged / 11\% declined / 13\% logged too late,
of 75 --- reported with its limitations (no control group; regression to the mean).
...
set); the gold labels are single-rater (a second Arabic-speaking rater is the planned upgrade to true
inter-rater \(\kappa\)); the effectiveness ``no-change'' bucket includes stable passing students, so
those percentages are directional; and the causal designs (RD at the cutoff, stepped wedge) are
pre-registered but unrun until Quiz-2 data exists.
\end{caveat}
...
loop (uplift / bandits) once volume justifies it. The single most important addition is the one-field
outcome log (``did the contact land?'') plus per-question quiz instrumentation --- moving the metric
from \emph{attempted} to \emph{effective}, and the diagnosis from ``at risk'' to ``weak on
quadratics''.
