...
\emph{reads} the Arabic facilitator notes (measured at \(\kappa=0.82\) against 75 human gold
...
\texttt{tests/test\_cast.py} & 22 acceptance tests (traps, guards, KPI invariants) & 193 \\
\bottomrule
\end{tabular}
\end{table}
...
with the gold labels at \(\kappa=0.82\) (88\% strict, 99\% lenient) --- and the eval earned its keep
...
\item \textbf{22/22 acceptance tests} pass --- the demo cast plus the system invariants: every
...
\item \textbf{Note-reader vs.\ human gold labels:} \(\kappa=0.82\), 88\% strict / 99\% lenient
